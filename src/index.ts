...
function extractDependenciesFromPackageJson(
...
function getDependencies(): [Set<string>, Set<string>] {
  ...
  try {
    ...
    for (const workspacePkg of workspacePaths) {
      const [wDeps, wDevDeps] =
        extractDependenciesFromPackageJson(workspacePkg);
...
async function suggestReason(
  ...
  if (
    depChain.toLowerCase().includes("devdependencies") ||
    depChain.toLowerCase().includes("dev-dep")
  ) {
    isDevDependency = true;
  }
  ...
  const severityColor = severities.some((s) => ["CRITICAL", "HIGH"].includes(s))
    ? chalk.red
    : chalk.yellow;
  console.log(`⚡ ${chalk.yellow("Sévérité:")} ${severityColor(severityStr)}`);

  console.log(`\n${chalk.yellow("Utilisé par:")}`);
  const depLines = depChain.includes(" | ")
    ? depChain.split(" | ")
    : depChain.split(" → ");
  ...
  while (true) {
    ...
    if (choice === "1") {
      ...
      console.log(
        chalk.red("Choix invalide, veuillez entrer un nombre entre 1 et 4"),
      );
    }
  }
}
...
async function parseVulnerabilities(
  scanResult: any,
  packageManager: PackageManager,
  ...
  for (const result of scanResult.results) {
    ...
    for (const pkg of result.packages) {
      ...
      if (vulnIds.length > 0) {
        const depChain = getDependencyChain(
          packageManager,
          pkgName,
          pkgVersion,
        );
...
function verifyConfig(lockfile: string): boolean {
  try {
    ...
    const result = spawnSync(
      "osv-scanner",
      [`--lockfile=${lockfile}`, "--config=osv-scanner.toml"],
      {
        encoding: "utf-8",
      },
    );
    ...
    if (result.status === 0) {
      printSuccess(
        "Aucune vulnérabilité détectée avec la nouvelle configuration",
      );
      return true;
    } else {
...
function generateReport(
  vulnerabilities: Vulnerability[],
  outputFile: string = "osv-scanner-report.md",
): void {
  const timestamp = new Date().toISOString().replace("T", " ").substring(0, 19);
...
async function main() {
  ...
  if (!checkOsvScanner()) {
    ...
    printInfo(
      "Installation: go install github.com/google/osv-scanner/cmd/osv-scanner@latest",
    );
    process.exit(1);
  }
  ...
  if (configExists) {
    if (!options.dryRun) {
      if (
        await askConfirmation(
          "Un fichier osv-scanner.toml existe déjà. Voulez-vous le sauvegarder ?",
          true,
        )
      ) {
    ...
    if (!options.dryRun) {
      if (
        !(await askConfirmation("Créer le fichier osv-scanner.toml ?", false))
      ) {
  ...
  console.log(
    `📦 Package manager détecté: ${chalk.cyan(packageManager.name)} v${pmVersion}`,
  );
  ...
  const vulnerabilities = await parseVulnerabilities(
    scanResult,
    packageManager,
  );
  ...
  const uniquePackages = new Set(
    vulnerabilities.map((v) => `${v.name}@${v.version}`),
  ).size;
  console.log(
    `🔍 ${vulnerabilities.length} vulnérabilités détectées dans ${uniquePackages} packages`,
  );
  ...
  if (options.dryRun) {
    ...
    const [configContent, filteredVulns] = generateTomlConfig(
      vulnerabilities,
      packageManager,
    );
    fs.writeFileSync("osv-scanner.toml", configContent, "utf-8");
    printSuccess(
      `Fichier osv-scanner.toml créé avec ${filteredVulns.length} vulnérabilités ignorées`,
    );
